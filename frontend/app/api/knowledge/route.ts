...
export async function GET() {
  ...
  const payload = {
    ...
    rooms: rooms.map((room) => ({
      ...
      // номер.
      //
      // Пути достраиваем до полных. Часть снимков лежит в хранилище и уже
      // хранится полным адресом, а часть — файлами самого сайта, и там путь
      // относительный вида `/images/rooms/...`. Мессенджер забирает картинку
      // по ссылке со своей стороны, и относительный путь ему некуда
      // приложить: такой номер молча уходил бы без фотографий.
      images: (room.images ?? [])
        .slice(0, 3)
        .map((src: string) =>
