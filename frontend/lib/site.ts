...
export const site = {
  ...
  legal: {
    bin: "200640012670",
    // Счёт сменился с ForteBank на Kaspi (2026-09-02). Реквизиты видны в
    // подвале и на странице оплаты, и по ним же сверяются присланные чеки:
    // расходятся они — и оплата юрлица уходит на закрытый счёт.
    iik: "KZ11722S000048166255",
    bik: "CASPKZKA",
    bank: 'АО "Kaspi Bank"',
    kbe: "17",
  },
  ...
  policy: {
    ...
    // Условия отмены. Держим здесь, потому что их спрашивают в трёх местах —
    // на странице оплаты, в оферте и у консьержа в переписке, — и разойтись
    // им нельзя: гость сошлётся на любую из формулировок.
    //
    // Сроки возврата подтверждены поддержкой платёжной системы 2026-08-31:
    // возврат оформляется в день отмены, до карты идёт 1–7 рабочих дней и
    // зависит от банка гостя, а не от отеля. Раньше на сайте стояло «до 14
    // рабочих дней» — цифра, взятая наугад.
    cancellation:
      "Бесплатная отмена — не позднее чем за 24 часа до заезда. При более поздней отмене или незаезде удерживается стоимость первых суток",
    refund:
      "Возврат оформляется в день отмены брони. На карту деньги приходят за 1–7 рабочих дней — срок зависит от банка, выпустившего карту, а не от отеля",
  },
} as const;
