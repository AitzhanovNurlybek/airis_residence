...
# Свежая заявка обязана быть видна отелю из общего списка, а не только внутри
# карточки компании: иначе менеджер узнает о ней, только если зайдёт наугад.
curl -s -b "$T/ajar" -o "$T/list2.html" "$F/admin/kompanii"
check "$(has "$T/list2.html" 'Заявки компаний в работе')" "общий список заявок компаний на месте"
check "$(has "$T/list2.html" 'K-')" "в списке виден номер заявки"
