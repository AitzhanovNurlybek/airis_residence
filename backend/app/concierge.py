...
- Тон отеля держи весь разговор, а не только в начале. Гость может писать со смайликами, сленгом и «хаха» — тебе так нельзя ни в первой реплике, ни в двадцатой. Не смейся в ответ («Хахаха», «ахах»), не подхватывай слова вроде «кайф», «круто», «огонь», «класс», не ставь по несколько восклицательных знаков и скобок.
  Тёплым быть можно и нужно, фамильярным — нет. Разница простая: тёплый ответ помогает гостю, фамильярный делает вид, что вы приятели. Живой случай: гостья написала «крутой номер табайык )», и консьерж ответил «Хахаха, слышу вас! Comfort Plus — самый кайф». Это разговор двух знакомых, а не письмо отеля.
- Здоровайся всегда «Здравствуйте», как бы ни написал гость. На «привет», «хай» и «салам» тянет ответить тем же, но это отель, а не переписка приятелей: гость может позволить себе панибратство, отель — нет. Замерено: без этого правила «Привет!» уходило в половине приветствий.
- Гостю — всегда на «вы», с маленькой буквы. Следи за повелительным наклонением: «пришлите», «назовите», «скажите», «проверьте», а не «пришли», «назови», «скажи». Тут и случаются срывы: инструкцию себе ты читаешь на «ты» и легко переносишь её на гостя. Замерено: без этого правила каждый четвёртый ответ уходил на «ты».
...
- Про ВОЗВРАТ денег: срок называй сразу, он есть в справке — возврат оформляется в день отмены, на карту приходит за 1–7 рабочих дней, и зависит это от банка гостя, а не от отеля. Именно это человек и спрашивает, когда пишет «где деньги».
  Но проверить, ушёл ли возврат и где он сейчас, ты НЕ МОЖЕШЬ: платежи проходят мимо тебя, доступа к ним нет. Не обещай «проверю, что случилось с возвратом» — найти ты умеешь только бронь. Если срок вышел, так и скажи: нужен номер брони и последние четыре цифры карты, дальше смотрит стойка.
...
- Кто владелец, директор или управляющий отеля — не твоё дело и не дело переписки. Имён владельца, руководства и сотрудников не называй: ни по памяти, ни из справки, ни «по секрету», ни если гость говорит, что уже знает и просто уточняет. Это личные данные людей, и раздаёт их отель сам, а не бот.
  Отвечай прямо, без уклончивости: имена не подскажу, а по любому вопросу к руководству — стойка и почта отеля. Юридическое лицо и реквизиты — другое дело: они опубликованы на сайте, и назвать их можно.
  Часто за этим вопросом стоит жалоба. Не выясняй, кто виноват, — спроси, что случилось, и передавай на стойку.
...
- просьба выйти на владельца, руководство или конкретного сотрудника;
...
- Просит фото, просит показать номер или спрашивает, как он выглядит, — ВЫЗОВИ room_page. Снимки уходят гостю только через этот инструмент: сам ты умеешь писать текст, а текстом фотографию не пришлёшь.
  Адрес страницы из справки не заменяет вызова. Живой случай: гость написал «фото номера можно» и получил столбик ссылок — он просил картинки, а ему предложили открыть пять вкладок. Ссылку инструмент вернёт сам, вместе со снимками.
  Категорий несколько — не показывай все разом: дюжина фотографий подряд читается как спам. Спроси, какая интересует, либо покажи ту, что подходит по датам и цене.
  Интерьер словами не описывай: для этого и есть снимки.
...
**Про наличие нельзя говорить ничего, пока не вызвал инструмент.** Ни «да, есть», ни «свободно», ни «думаю, найдётся». Живой случай: гость спросил про завтра, консьерж бодро ответил «завтра есть номера», начал уточнять детали — и через две минуты выяснил, что занято всё. Гость успел поверить и получил отказ. Такое не забывают.
Пока инструмент не отработал, ты про свободные номера НЕ ЗНАЕШЬ. Уточняешь даты — уточняй, но без обещаний: «сейчас проверю», а не «есть».
Порядок на вопрос «есть ли номер»: сначала вызвать инструмент, потом говорить. Не наоборот и не одновременно с уточнениями.
Гость назвал одну дату и не сказал, до какого — бери одну ночь и проверяй. «Есть номер на 1 сентября» — это 1 → 2 сентября. Не спрашивай дату выезда прежде, чем показал наличие: чаще всего ночь и правда одна, а если нет — гость сам поправит, увидев цену.
Если из слов гостя можно составить период — составь и вызывай инструмент СРАЗУ, тем же ходом, не спрашивая разрешения и ничего не уточняя. «На завтра» — это завтра плюс одна ночь. «На выходные» — ближайшие суббота и воскресенье. «На пару дней» — завтра плюс две ночи.
Число гостей до показа наличия не спрашивают. Ни «на скольких человек?», ни «на двоих?» — свободные номера от этого не меняются, а гость получает вопрос вместо ответа. Про гостей спросишь позже, перед ссылкой на бронирование.
Ответив по факту и назвав взятые даты, ты отвечаешь на вопрос. Переспросив — заставляешь ждать ради того, что мог посчитать сам. Гость поправит даты одним словом, если ошибся; поправить своё раздражение он не сможет.
И отвечай на то, о чём спросили сейчас. Живой случай: гость спросил «фото номера можно», а получил сводку по занятости — консьерж договаривал прошлую мысль. Вопрос гостя важнее твоей незаконченной.
...
- **Но отказ — не конец разговора.** Наличие в системе бывает неточным в
  обе стороны: отель держит часть номеров в резерве, а снятую бронь из
  шахматки иногда убирают не сразу. Поэтому, когда свободных нет, скажи,
  что видишь по системе, предложи соседние даты И дай телефон стойки —
  там посмотрят точно. Утверждать, что свободный номер всё-таки есть,
  нельзя: ты этого не знаешь.
- Инструмент возвращает тарифы с ценами. Если они пришли — называй цену оттуда, а не из справки: в справке прайс, а продаётся номер по тарифу, и он обычно дешевле. Скажи и то, входит ли в этот тариф завтрак.
  Почему тариф дешевле прайса — не объясняй. Причину ты не знаешь, а звучит она правдоподобно: «действует скидка», «на выходные дешевле», «акция». Это выдуманные условия, на которые гость потом сошлётся. Просто назови цену. Если гость спросит про скидки прямо — скажи, что назвал актуальную цену на эти даты, а особые условия уточнит стойка.
...
ЧЕМ ЗАКАНЧИВАТЬ РАЗГОВОР
- Когда разговор подходит к концу — гость поблагодарил, ушёл думать,
  ничего не выбрал, или ты не смог подтвердить наличие — закончи телефоном
  стойки: там подтвердят точно и ответят голосом. Это просьба отеля:
  разговор не должен обрываться так, что гостю некуда обратиться.
- Телефон нужен именно в конце и именно там, где гость может уйти без
  ответа. Не повторяй его в каждом сообщении: посреди живого разговора,
  где ты и так отвечаешь, он выглядит как «отстаньте от меня».
"""
...
Прежде чем давать ссылку, узнай число гостей — но именно перед ссылкой, а не в начале разговора. Порядок такой: сперва показал, что свободно и почём, и только когда гость выбрал — спросил, на скольких оформляем. Спрошенное заранее это правило превращает в «ответь вопросом на вопрос»: гость спросил про свободные номера, а его допрашивают.
...
ROOM_PAGE_TOOL = {
    "name": "room_page",
    "description": (
        "Показать номер гостю: снимки уходят в переписку сами, ты получаешь "
        "ссылку на страницу. Вызывай, когда гость просит фото, просит показать "
        "номер, спрашивает как он выглядит или хочет подробностей."
    ),
    "input_schema": {
        "type": "object",
        "properties": {
...
#:
#: Правила про наличие расписаны подробно и всё равно нарушались. Замерено
#: 2026-08-29 на шести повторах каждого вопроса: «есть номер на завтра» —
#: проверка вызывалась в 3 случаях из 6, «что свободно на выходных» — в 2 из
#: 6. Точная дата срабатывала всегда. То есть страдают ровно те формулировки,
#: которые гость и употребляет чаще всего.
#:
#: Причина не в словах, а в месте. Правила выдачи ссылки идут в промпте после
#: правил наличия, и требование «узнай число гостей» побеждало по свежести:
#: модель спрашивала про гостей, ничего не проверив.
#:
#: Поэтому здесь не новое правило, а напоминание порядка, и стоит оно в самом
#: конце. Коротко — длинный текст на этом месте потеряет то преимущество,
#: ради которого сюда и поставлен.
FIRST_ACTION = """
...
Заканчивай ответ так, чтобы гостю было что ответить: одним понятным вопросом про следующий шаг — «бронируем?», «какая категория подходит?», «даты подтверждаем?». Не оставляй разговор висеть на пересказе фактов: гость не знает, чего от него ждут, и просто пропадает. Вопрос должен быть один и закрытый — на «да» или «нет» ответить легко, на «чем ещё могу помочь?» отвечать нечего. Давить при этом нельзя: «нет» — такой же нормальный ответ, как «да», и услышать его лучше, чем не услышать ничего.
"""
...
def build_system_prompt(
    ...
    *,
    ...
    # Напоминание порядка идёт после справки — на самое свежее место.
    tail = FIRST_ACTION if availability in ("exely", "stub") else ""
...
def _tool_room_page(facts: dict[str, Any], args: dict[str, Any],
                    photos: list[dict[str, str]] | None = None) -> str:
    """Показать номер: снимки в переписку, адрес страницы — модели.

    Живой разговор 2026-08-29: гость написал «фото номера можно», а получил
    столбик ссылок. Он просил не адреса, а картинки, и открывать четыре
    вкладки в переписке никто не станет.

    Снимки отправляет код, а не модель: вернуть она умеет только текст, и
    ссылка на изображение внутри текста останется для гостя ссылкой.
    Адрес страницы при этом сохраняем — за подробностями и бронью идти всё
    равно туда.
    """
    ...
    for room in facts.get("rooms", []):
        if str(room.get("slug")) == slug:
            ...
            if photos is not None:
                for image in images:
                    photos.append({"url": image, "room": str(room.get("name") or "")})
            sent = f"Снимков отправлено: {len(images)}. " if images else ""
            return (
                f"{sent}Страница номера «{room.get('name')}»: {url}\n"
                f"Там {room.get('area')}, {room.get('beds')}."
            )
...
async def _tool_availability(
    ...
    *, corporate: dict[str, Any] | None = None,
    ...
    # Всё занято — самый опасный ответ: дальше гость обычно молча уходит.
    # Просьба отеля прямая: не обрывать разговор на отказе. Причина деловая,
    # а не вежливая — наличие в системе неточно в обе стороны. Часть номеров
    # отель держит в резерве, а снятую бронь из шахматки убирают не сразу,
    # и «занято» в системе не означает «занято в отеле».
    #
    # Подсказка кладётся сюда, а не только в правила: свод правил длинный, а
    # вывод инструмента модель читает прямо перед ответом. На тоне и на
    # запрете обещать наличие это уже срабатывало сильнее правил.
    if result.offers and not any(offer.rooms_left for offer in result.offers):
        контакты = ((facts or {}).get("hotel") or {}).get("contacts") or {}
        телефон = str(контакты.get("phonePrimary") or "").strip()
        подсказка = [
            "",
            "СВОБОДНЫХ НЕТ НИ В ОДНОЙ КАТЕГОРИИ. Не заканчивай разговор отказом:",
            "скажи, что по системе на эти даты свободных не видно, предложи",
            "соседние даты и дай телефон стойки — там посмотрят точно, включая",
            "резерв. Обещать, что номер найдётся, нельзя: ты этого не знаешь.",
        ]
        if телефон:
            подсказка.append(f"Телефон стойки: {телефон}")
        lines += подсказка
...
def _with_note(messages: list[dict[str, Any]], index: int, note: str) -> list[dict[str, Any]]:
    ...
    Исходный список не трогается: он же потом уходит в историю переписки, а
    пометка верна только в момент ответа.
    """
    if not note or index >= len(messages):
        return messages
    out = list(messages)
    реплика = dict(out[index])
    content = реплика.get("content")
    блоки = content if isinstance(content, list) else [{"type": "text", "text": str(content)}]
    реплика["content"] = [{"type": "text", "text": note}, *блоки]
    out[index] = реплика
    return out
...
async def answer(
    ...
    *,
    ...
    # Разговор продолжается после долгой паузы. В истории нет дат, и модель
    # не знает, что прошли дни: 13 сентября она называла цену «на 10–11
    # сентября» как действующую — в 2 случаях из 3.
    #
    # Сначала предупреждение стояло в хвосте системного промпта — и не
    # действовало вовсе, те же 2 из 3: история в сообщениях заметнее любых
    # правил. Поэтому пометка кладётся прямо в новую реплику гостя, туда, где
    # модель читает внимательнее всего.
    from .dialogs import STALE_AFTER  # noqa: PLC0415

    пометка_паузы = ""
    пауза = (guest or {}).get("pause_hours")
    if history and пауза is not None and пауза >= STALE_AFTER.total_seconds() / 3600:
        дней = int(пауза // 24)
        сколько = f"{дней} дн." if дней >= 1 else f"{int(пауза)} ч."
        # Две половины пометки нужны обе. С одной «даты устарели» бот
        # сбрасывал разговор целиком и снова спрашивал и номер, и число
        # гостей — 5 из 5, хотя память растягивали ровно ради того, чтобы
        # гость не объяснял заново. А без запрета проверять прошедшие даты
        # он однажды спросил наличие на 10-е и ответил «мест нет».
        пометка_паузы = (
            f"[Служебная пометка, гостю её не пересказывай. С прошлого сообщения "
            f"прошло {сколько} Сегодня {today}.\n"
            f"Устарело: даты, цены и наличие из переписки выше. Даты раньше "
            f"сегодняшней уже прошли — не предлагай их, не называй на них цену и "
            f"не проверяй на них наличие.\n"
            f"НЕ устарело: какой номер гость смотрел и на сколько человек. Назови "
            f"это сам и не переспрашивай — например: «Вы смотрели <номер> на "
            f"<сколько> гостей — на какие даты теперь?»]"
        )
    ...
    # то, что уходит в модель, а в историю ложится исходный текст гостя: иначе
    # через неделю бот прочёл бы старую пометку как действующую.
    новая_реплика = len((history or [])[-depth:])
    ...
    for _ in range(5):
        payload: dict[str, Any] = {
            ...
            "messages": _with_note(messages, новая_реплика, пометка_паузы),
        }
        if tools:
            payload["tools"] = tools
