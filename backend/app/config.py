...
class Settings(BaseSettings):
    ...
    # Куда слать заявки с сайта в WhatsApp. Пусто — на номер самого бота, в
    # чат «Сообщение для себя»: он есть всегда и никуда не денется при смене
    # сотрудников. Явный номер нужен, когда заявки должен видеть менеджер, а
    # не владелец.
    #
    # Появилось потому, что Telegram у отеля не настроен, а заявки уходили
    # только туда: шесть штук пролежали в базе непрочитанными, у четырёх дата
