...
class Settings(BaseSettings):
    ...
    #
    # Заявки с сайта и расчёт возврата уходят всегда, независимо от этой
    # настройки: про них Exely не знает ничего.
    notify_bookings: str = ""

    # Возвращать ли деньги при отмене брони самостоятельно.
    #
    # По умолчанию НЕТ, и это не осторожность ради осторожности: связки между
    # Exely и банком не существует (прямой ответ поддержки Exely), возврат
    # оформляет бухгалтерия руками, и подменять человека в движении чужих
    # денег без его ведома нельзя. При выключенном флаге отель получает
    # готовый расчёт — бронь, платёж, сумма, — и жмёт одну кнопку вместо
    # получаса поисков в кабинете банка.
    refund_auto: bool = False

    # Предел суммы для автоматического возврата. Ноль — без предела.
    # Защита не от отеля, а от опечатки в правилах отмены: одна лишняя цифра
    # не должна уходить в банк без единого человеческого взгляда.
    refund_max: int = 100000
