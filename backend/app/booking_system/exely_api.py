...
class ExelyApi:
    ...
    async def find_bookings(self, *, phone: str = "", name: str = "") -> list[ExternalBooking]:
        """Поиск броней гостя по телефону.

        **Всегда возвращает пусто, и это не заглушка, а свойство API.**

        Read Reservation API не отдаёт ни телефона, ни почты гостя — ни в
        сводке (там только номер, статус и даты изменения), ни в полной
        брони (там `customer` — это ровно firstName и lastName). Проверено
        на живой броне 2026-08-29: строки с телефоном в ответе нет вовсе,
        хотя в ваучере гостю телефон печатается.

        Раньше это выглядело как «у вас нет активных броней»: код честно
        фильтровал по телефону, ничего не находил и отвечал гостю, что
        броней нет — при живой броне. Хуже молчаливой ошибки не придумаешь.

        Искать бронь надо по её номеру (`get_booking`), он есть у гостя в
        подтверждении. Здесь оставлено пустое возвращение, чтобы вызывающий
        код не падал, но правила консьержа теперь просят номер брони, а не
        обещают найти по телефону.
        """
        logger.info(
            "Поиск брони по телефону невозможен: Exely не отдаёт контакты "
            "гостя. Нужен номер брони."
        )
        return []
