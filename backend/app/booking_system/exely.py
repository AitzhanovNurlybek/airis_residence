...
class ExelyBookingSystem:
    ...
    #: Искать брони по телефону Exely не позволяет: Read Reservation API не
    #: отдаёт контактов гостя вовсе. Значит остаётся номер брони — и по нему
    #: бронь выдавать можно: он длинный и есть только у самого гостя.
